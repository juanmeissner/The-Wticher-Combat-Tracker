...
const styleSource = fs.readFileSync(path.join(projectRoot, 'style.css'), 'latin1');
...
assert.match(renderSource, /function captureCombatListViewport/);
assert.match(renderSource, /function restoreCombatListViewport/);
assert.match(renderSource, /restoreCombatListViewport\(container, preservedViewport\)/);
assert.match(renderSource, /requestAnimationFrame\(\(\) =>/);
assert.match(renderSource, /viewportRestoreRevision !== combatViewportRestoreRevision/);
assert.match(renderSource, /function releaseCombatPanelFocus/);
assert.match(renderSource, /activeElement\.closest\?\.\('\.combat-subpanel-header'\)/);
assert.match(styleSource, /\.combat-list\s*\{[\s\S]*?overflow-anchor:\s*none;/);
